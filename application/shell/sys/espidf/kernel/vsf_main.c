...
// Backing store for the shim's root mal. A flat 64 KiB window hosts the
// four compile-time partitions: nvs/phy/storage live in the first 2 KiB
// (0x0000/0x0400/0x0800 -- untouched by littlefs/fatfs to keep the
// esp_partition RW test suite stable across runs), a 32 KiB "lfs"
// partition at 0x1000 is reserved for the esp_vfs_littlefs end-to-end
// mount test (erase + format + open/write/read + persistence across
// remount), and the tail is left free so esp_partition_register_external()
// can park a dynamic "ext0" slice at 0xE000 without overlapping any
// static entry.
//
// On this Windows host we route the root mal through vk_file_mal_t: all
// partition I/O ultimately lands in ./root/nvs_flash.bin via the winfs
// VFS driver. This is a test-application-only policy -- the ESP-IDF
// shim itself only sees a generic vk_mal_t* (the mim_mal slices it
// creates). Any other platform would inject e.g. a real flash_mal with
// identical API surface.
//
// file_mal exposes READ+WRITE+ERASE (erase emulates 0xFF fill on the
// underlying file, see vsf_file_mal.c). LOCAL_BUFFER is absent because
// there is no direct-addressable window -- the partition mmap test keys
// off this bit to pick its expectation.
#define __PART_ROOT_SIZE        (64u * 1024u)
#define __PART_ROOT_BLKSZ       256u
#define __PART_ROOT_FILE        "/root/nvs_flash.bin"
static vk_file_mal_t __part_root_mal = { 0 };
static int           __part_root_fd  = -1;

// Compile-time partition table. Offsets/sizes are block-aligned on
// __PART_ROOT_BLKSZ so every esp_partition_read/write target stays on
// a clean boundary. Labels follow the ESP-IDF convention (nvs/phy/
// storage) to make the test output readable.
static const vsf_espidf_partition_entry_t __part_entries[] = {
    {
        .label      = "nvs",
...
static void __part_root_setup(void)
{
    // Windows persistence path: mount winfs at /root so that the VSF
    // VFS layer maps /root/** to the literal Windows directory ./root
    // (the host directory is kept under source control and must exist
    // before boot; winfs __vk_winfs_mount() rejects the mount otherwise).
    // vsf_linux_create_fhs() overrides the weak default and thus skips
    // the canonical vsf_linux_vfs_init() call; do it manually here so
    // POSIX mount/open/ftruncate work.
    vsf_linux_vfs_init();
    ...
    if (mount(NULL, "root", &vk_winfs_op, 0, (const void *)&__root_fs) != 0) {
        vsf_trace_error("esp_partition: failed to mount winfs at /root"
                        VSF_TRACE_CFG_LINEEND);
        return;
    }

    // Open (create if missing) the synthetic flash image. We pre-size it
    // to __PART_ROOT_SIZE once at boot so vk_file_mal_init() can infer
    // pthis->size from file->size. A partially-truncated image from a
    // previous run is extended (never shrunk) so already-committed NVS
    // data survives; a fresh image starts filled with 0x00.
    __part_root_fd = open(__PART_ROOT_FILE, O_RDWR | O_CREAT, 0666);
    if (__part_root_fd < 0) {
        vsf_trace_error("esp_partition: failed to open " __PART_ROOT_FILE
                        VSF_TRACE_CFG_LINEEND);
        return;
    }
    struct stat st;
    if ((fstat(__part_root_fd, &st) == 0) && (st.st_size < __PART_ROOT_SIZE)) {
        if (ftruncate(__part_root_fd, __PART_ROOT_SIZE) != 0) {
            vsf_trace_error("esp_partition: failed to grow " __PART_ROOT_FILE
                            " to %u bytes" VSF_TRACE_CFG_LINEEND,
                            (unsigned)__PART_ROOT_SIZE);
        }
    }

    // POSIX fd -> underlying VSF file handle. The linux shim exposes
    // __vsf_linux_get_fs_ex() for exactly this kind of bridge scenario.
    // The fd stays open for the process lifetime; vk_file_mal_t takes a
    // raw pointer so we must keep the file alive.
    extern vk_file_t * __vsf_linux_get_fs_ex(vsf_linux_process_t *process, int fd);
    vk_file_t *vkfile = __vsf_linux_get_fs_ex(NULL, __part_root_fd);
    if (vkfile == NULL) {
        vsf_trace_error("esp_partition: fd->vk_file lookup failed"
                        VSF_TRACE_CFG_LINEEND);
        return;
    }

    // Wire the root mal. file_mal now implements an erase entry that
    // streams 0xFF over the requested range, so VSF_MAL_ERASABLE is set
    // alongside READ/WRITE. VSF_MAL_LOCAL_BUFFER stays off (file-backed:
    // no direct window). Downstream mim_mal slices inherit this feature
    // bit-for-bit inside esp_partition_port.c.
    __part_root_mal.drv        = &vk_file_mal_drv;
    __part_root_mal.feature    = VSF_MAL_READABLE | VSF_MAL_WRITABLE
                               | VSF_MAL_ERASABLE;
    __part_root_mal.file       = vkfile;
    __part_root_mal.block_size = __PART_ROOT_BLKSZ;

    // file_mal's init is synchronous on the peda path when invoked from
    // a stack-owning thread; it just reads file->size and computes the
    // block-aligned usable window into pthis->size.
    (void)vk_mal_init((vk_mal_t *)&__part_root_mal);
}
...
static void __run_espidf_tests(void)
{
    __test_total = 0;
    __test_pass  = 0;
...
static void * __espidf_test_entry(void *arg)
{
    (void)arg;
    __run_espidf_tests();
...
{
    // ESP-IDF shim configuration. The cfg struct is purely input and
    // can live on the stack -- the shim copies what it needs internally.
    // Peripheral pointers are left NULL on this host build; a target
    // board would populate e.g. .rng with a vsf_multiplex_rng_t cast to
    // vsf_rng_t*.
    vsf_trace_info("start espidf ..." VSF_TRACE_CFG_LINEEND);

    // Bring up the file_mal-backed synthetic flash first so the shim
    // can slice compile-time partitions off it during vsf_espidf_init.
    __part_root_setup();

    vsf_espidf_cfg_t cfg = { 0 };
    cfg.partition.root_mal    = (vk_mal_t *)&__part_root_mal;
    cfg.partition.entries     = __part_entries;
    cfg.partition.entry_count = dimof(__part_entries);
    vsf_trace_info("espidf: before vsf_espidf_init"VSF_TRACE_CFG_LINEEND);
    vsf_espidf_init(&cfg);
    vsf_trace_info("espidf: after vsf_espidf_init"VSF_TRACE_CFG_LINEEND);
    vsf_freertos_init();
    vsf_trace_info("espidf: after vsf_freertos_init"VSF_TRACE_CFG_LINEEND);
    ...
    pthread_create(&tid, &attr, __espidf_test_entry, NULL);
